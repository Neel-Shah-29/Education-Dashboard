#include "client.hpp"
/*
Handles the request for displaying the
submissions for
...
void make_submission(user usr)
{
    ...
    cin >> file_name;
    string to_send = category + delim +
                     assignment_name + delim + file_name;
    int len = to_send.length();
...
/*
Handles the student's request for joining a live
chat session
...
/*
Handles the student's request for downloading the
attachments
...
delim + assignment_name + delim + filename;
int len = to_send.length();
